...
class Chunk:
    ...
    def unpack(raw_nbt):
        ...
        for section in raw_nbt.get('Level').children:
...
class World:
    def __init__(self, world_folder, save_location=None, debug=True, read=True, write=True):
        self.debug = debug
    ...
    def _load_binary_chunk_at(self, region_file, offset, max_size):
        ...
        chunk_pos = (data.get('xPos'), data.get('zPos'))
        chunk = Chunk(
            chunk_pos[0],
            chunk_pos[1],
            Chunk.unpack(data),
    ...
    def _get_region(self, chunk_pos):
        result = math.floor(chunk_pos[0] / 32), math.floor(chunk_pos[1] / 32)
        print(str(result))
        return (math.floor(chunk_pos[0] / 32) - 1, math.floor(chunk_pos[1] / 32) - 2)
