...
def read_level_from_file(input_dir, input_name, coords, block2repr, repr_type, debug=False):
    ...
    build_array = None 
    with h5py.File(os.path.join(input_dir, input_name), "r") as file:
        keys = file.keys()
        if len(keys) == 0:
            logger.info("%s failed loading: no keys." % input_name)
            exit(1)
        else: 
            build_array = np.array(file[list(keys)[0]][()], dtype=np.int32)
            logger.info("%s loaded." % input_name)  
    ...
    # Inject
    pattern = r'\[(.*?)\]'
    with open(Config.tok2block_filepath, "r") as file:
